...
def split_load_mnist(batch_size: int=32, num_context_points=None, uniform_sampling = True) -> Tuple[tf.data.Dataset, tf.data.Dataset]:
    """Loads the mnist dataset
    ...
    def encode(element):
        ...
        img = tf.cast(element['image'], tf.float32) / 255. # normalise pixels within [0,1] range
        global prev_image
        new_img = tf.concat((img[0:14, :, :], prev_image[14:, :, :]), axis=0)
        prev_image = new_img
    ...
    train_ds = train_ds.map(encode).prefetch(tf.data.experimental.AUTOTUNE)
    #test_ds = test_ds.map(encode).prefetch(tf.data.experimental.AUTOTUNE)  # TODO check if prefetch needed
...
(context_x, context_y, target_x), target_y = encode(next(ds))
