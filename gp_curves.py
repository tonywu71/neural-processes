...
class GPCurvesGenerator:
    ...
    def generate(self):
        ...
        """
        n_context = tf.random.uniform([], 3, self.max_size, dtype=tf.int32)
        ...
        if self.testing:
            ...
            n_target = tf.random.uniform([], 3, self.max_size, dtype=tf.int32)
            n_total = n_target + n_context
            # x : [B, n_total, self.x_dim]
            x = tf.random.uniform([self.batch_size, n_total, self.x_dim], self.x_start, self.x_end)
        
        param_shape = [self.batch_size, self.y_dim, self.x_dim]
        if self.random_kernel_parameters:
            l1 = tf.random.uniform(param_shape, 0.1, self.l1_scale)
            sigma = tf.random.uniform(param_shape[:-1], 0.1, self.sigma_scale)
        else:
            l1 = tf.ones(param_shape) * self.l1_scale
            sigma = tf.ones(param_shape[:-1]) * self.sigma_scale
        
        kernel = self.gaussian_kernels(x, l1, sigma)
        cholesky = tf.linalg.cholesky(kernel)

        y = cholesky @ tf.random.normal([self.batch_size, self.y_dim, n_total, 1])
